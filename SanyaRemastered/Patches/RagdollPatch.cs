...
namespace SanyaRemastered.Patches
{
    ...
    public static class PreventRagdollPatch
    {
        public static bool Prefix(RagdollManager __instance, Vector3 pos, Quaternion rot, Vector3 velocity, int classId, PlayerStats.HitInfo ragdollInfo, bool allowRecall, string ownerID, string ownerNick, int playerId, bool _096Death = false)
        {
            ...
            Player Owner = Player.Get(playerId);
            if (Owner.TryGetSessionVariable("NewRole", out System.Tuple<string, string> newrole))
                role.fullName = newrole.Item2;
            gameObject.transform.localScale = new Vector3(Owner.Scale.x * gameObject.transform.localScale.x,
                                                          Owner.Scale.y * gameObject.transform.localScale.y,
                                                          Owner.Scale.z * gameObject.transform.localScale.z);
