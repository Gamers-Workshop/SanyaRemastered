		...
		public static void Prefix(Intercom __instance)
		{
            try
            {
                ...
                if (Room.Get(RoomType.Hcz106)?.LightsOn ?? true)
                    if (Scp106Container.AllowContain)
                    {
                        if (Scp106Container.Scp106FemurUsed)
                        {
                            stringBuilder.Append($"Statut du briseur de fémur : <color=#228B22>Utilisé</color>\n");
                        }
                        else
                        {
                            stringBuilder.Append($"<color=#ff0000>Statut du briseur de fémur : Prêt</color>\n");
                        }
                    }
                    else
                    {
                        stringBuilder.Append($"Statut du briseur de fémur : Vide\n");
                    }
                else
                {
                    stringBuilder.Append($"Statut du briseur de fémur : <color=#228B22>No Data</color>\n");
                }
                ...
                else if (__instance.speaker is not null)
                {
                    stringBuilder.Append($"{Player.Get(__instance.Networkspeaker)?.DisplayNickname} Diffuse : {Mathf.CeilToInt(__instance.speechRemainingTime)} secondes ");
                }
                else
                {
                    stringBuilder.Append("Intercom prêt à l'emploi.");
                }
                __instance.CustomContent = stringBuilder.ToString();
                stringBuilder.Clear();
                return;
            }
            catch (Exception ex)
